...
pyZ3, like Zig, doesn't perform any implicit memory management. pyZ3 is designed to be a relatively
thin layer around the CPython API, and therefore the same semantics apply.

All pyZ3 Python types (such as `py.PyObject` and `py.Py<Name>(root)`) have `incref()` and `decref()` member
...
In general, pyZ3 functions do not steal references. They should be loudly documented in the rare cases that 
...
!!! tip "Upcoming Feature!"
    ...
    catch memory leaks within your pyZ3 extension code and surface them to pytest.
