...
In pyZ3, all Python declarations start life as a struct. When a struct is registered with
pyZ3 as a module, a `#!c PyObject *PyInit_<modulename>(void)` function is created automatically
and exported from the compiled shared library. This allows the module to be imported by Python.

## Example Module

Please refer to the annotations in this example module for an explanation of the pyZ3 features.
...
5. Arguments in pyZ3 are accepted as a pointer to a const struct. This allows pyZ3 to generate
...
7. All modules must be registered with pyZ3 such that a `PyInit_<modulename>` function is
