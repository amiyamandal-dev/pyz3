...
This page describes how to define, instantiate and customize Python classes with pyZ3.
...
By default, pyZ3 classes can _only_ be instantiated from Zig. While it is possible to
create pyZ3 structs with Zig syntax, this will only create a Zig struct and not the
...
Declaring a `__init__` function signifies to pyZ3 to make your class instantiable
...
Inheritance allows you to define a subclass of another Zig pyZ3 class.
...
pyZ3 properties are again defined as structs with optional `get` and `set` methods. If you
...
- `object` refers to either a pointer to a pyZ3 type, a `py.PyObject(roo)`,
  or any other pyZ3 Python type, e.g. `py.PyString`.
...
pyZ3 also defines `py.CompareOp` representing the CPython comparison operators allowing you
