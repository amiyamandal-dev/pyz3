...
pyZ3 provides two functions for managing GIL state: `py.nogil` and `py.gil`.

## No GIL / Allow Threads

The `py.nogil` function allows pyZ3 code to release the Python GIL. This allows Python threads to continue to
...
The `py.gil` function allows pyZ3 code to re-acquire the Python GIL before calling back into Python code.
