...
def test_sum_list(example):
    ...
    result = example.list_conversion_example.sum_list(numbers=[1, 2, 3, 4, 5])
    assert result == 15


def test_create_range(example):
    """Test creating Python list from Zig array"""
    result = example.list_conversion_example.create_range(start=0, end=5)
...
def test_minmax_empty_list(example):
    ...
    with pytest.raises(ValueError, match="List cannot be empty"):
        example.list_conversion_example.minmax(numbers=[])
...
def test_create_matrix(example):
    ...
    result = example.list_conversion_example.create_matrix(rows=2, cols=3)
    assert result == [
        [1, 2, 3],
        [4, 5, 6]
    ]
...
def test_filter_above(example):
    ...
    result = example.list_conversion_example.filter_above(
        numbers=[1.5, 3.2, 2.1, 4.8, 1.9, 5.1],
        threshold=3.0
    )
...
def test_empty_list(example):
    ...
    result = example.list_conversion_example.sum_list(numbers=[])
    assert result == 0


def test_single_element(example):
    """Test single element list"""
    result = example.list_conversion_example.sum_list(numbers=[42])
    assert result == 42


def test_floats(example):
    """Test float list conversion"""
    result = example.list_conversion_example.sum_list(numbers=[1.5, 2.5, 3.0])
    # Note: Will be truncated to integers
